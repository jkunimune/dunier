...
import { binarySearch, cumulativeIntegral, gradient_linterp, linterp, localizeInRange } from "../utilities/miscellaneus.js";
...
var MapProjection = /** @class */ (function () {
     ...
     */
    function MapProjection(surface, φStandard, φRef, yRef, dx_dλRef, yCenter, λCenter) {
        var e_1, _a, e_2, _b;
        var _this = this;
        // check the inputs
        ...
        this.domain = new Domain((this.φMin + this.φMax) / 2 - Math.PI, (this.φMin + this.φMax) / 2 - Math.PI + 2 * Math.PI, // this weird phrasing is to ensure the diff is always 2π even when there's roundoff
        this.λMin, this.λMax, function (point) { return _this.surface.isOnEdge(assert_φλ(point)); });
     ...
     */
    MapProjection.prototype.recenter = function (λCenter) {
        return new MapProjection(this.surface, this.φStandard, this.φRef, this.yRef, this.dx_dλRef, this.yCenter, λCenter);
    };
    /**
    ...
    MapProjection.prototype.gradient = function (point) {
        if (isFinite(this.yCenter)) {
            var r = this.y(point.φ) - this.yCenter;
            var Δλ = point.λ - this.λCenter;
            if (point.λ === this.λMin || point.λ === this.λMax)
                Δλ = Math.sign(Δλ) * Math.PI; // set longitude to exactly ±π if it seems like it should be
            var dr_dφ = this.dy_dφ(point.φ);
            var ds_dλ = this.dx_dλ(point.φ);
            var θ = (r !== 0) ? ds_dλ / r * Δλ : 0;
            var ds_dφ = this.dx2_dλdφ(point.φ) * Δλ - dr_dφ * θ;
            return {
                φ: {
     ...
     */
    MapProjection.prototype.hasStraightMeridians = function () {
        var φ0 = (this.φMin + this.φMax) / 2;
        var θ = this.dx_dλ(φ0) / (this.y(φ0) - this.yCenter);
        if (θ !== 0) {
            for (var i = 0; i < this.φRef.length; i++)
            ...
            for (var i = 1; i < this.φRef.length; i++)
     ...
     */
    MapProjection.prototype.dy_dφ = function (φ) {
     ...
     * @private the derivative of y in kilometers/radian
     */
    MapProjection.prototype.dx2_dλdφ = function (φ) {
    ...
    MapProjection.orthographic = function (surface, φMin, φMax, λStd) {
        ...
        return new MapProjection(surface, (φMin + φMax) / 2, φ, R.map(function (x) { return -x; }), R, 0, λStd);
    };
    /**
