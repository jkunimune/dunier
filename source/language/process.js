 ...
 */
var SoundChange = /** @class */ (function () {
    function SoundChange(from, to, idx, after, before) {
     ...
     * @param oldWord
     */
    SoundChange.prototype.apply = function (oldWord) {
...
var Harmony = /** @class */ (function () {
    ...
    Harmony.prototype.apply = function (oldWord) {
        var e_5, _a;
        ...
        for (var i = 0; i < oldWord.length; i++) { // iterate backwards through the word
            ...
            if (oldWord[i].is(this.affected)) { // but if this segment isn't immune
                try {
                    for (var _b = (e_5 = void 0, __values(this.poles)), _c = _b.next(); !_c.done; _c = _b.next()) { // check its polarity
 ...
 */
var Syllabicization = /** @class */ (function () {
    /**
    ...
    Syllabicization.prototype.apply = function (oldWord) {
        var e_6, _a;
        var sonority = [];
        try {
 ...
 */
var StressPlacement = /** @class */ (function () {
    /**
    ...
    StressPlacement.prototype.apply = function (oldWord) {
        var nuclei = [];
        var numC = 1;
 ...
 */
var PhraseProcess = /** @class */ (function () {
    /**
     * create a new change; just specify the name and we'll know what to do.
     */
    function PhraseProcess(name) {
        this.name = name;
    }
    PhraseProcess.prototype.apply = function (old) {
        var e_7, _a;
        if (this.name === "compounding") {
            // string all of the parts together without pauses, and make sure there's only one primary stress
            var compoundWord = [];
            for (var i = 0; i < old.parts.length - 1; i++) {
                try {
                    for (var _b = (e_7 = void 0, __values(old.parts[i])), _c = _b.next(); !_c.done; _c = _b.next()) {
            ...
            compoundWord.push.apply(compoundWord, __spreadArray([], __read(old.parts[old.parts.length - 1]), false));
            return new Name([compoundWord], old.language);
        }
        else
            throw new Error("the special process ".concat(this.name, " is not implemented."));
    };
    return PhraseProcess;
}());
export { PhraseProcess };
export var DEFAULT_STRESS = new StressPlacement(true, 1, 1, 'lapse', false);
export var WORD_PROCESS_OPTIONS = [];
export var PHRASE_PROCESS_OPTIONS = [];
try {
    for (var UNPARSED_PROCESS_OPTIONS_1 = __values(UNPARSED_PROCESS_OPTIONS), UNPARSED_PROCESS_OPTIONS_1_1 = UNPARSED_PROCESS_OPTIONS_1.next(); !UNPARSED_PROCESS_OPTIONS_1_1.done; UNPARSED_PROCESS_OPTIONS_1_1 = UNPARSED_PROCESS_OPTIONS_1.next()) { // load the phonological processes
        ...
        if (type === 'mute') {
            ...
            WORD_PROCESS_OPTIONS.push({ chanse: chance, proces: new SoundChange(ca, pa, idx, bada, chena) });
        }
        else if (type === 'harmonia') {
            var _h = __read(code.split(" "), 4), leftPole = _h[0], _ = _h[1], rightPole = _h[2], scope = _h[3];
            WORD_PROCESS_OPTIONS.push({ chanse: chance, proces: new Harmony(parseFeature(leftPole), parseFeature(rightPole), parseFeature(scope)) });
        }
        else if (type === 'acente') {
            var _j = __read(code.split(" "), 2), reverse = _j[0], headSize = _j[1];
                    ...
                    for (var _k = (e_3 = void 0, __values(['clash', 'lapse', 'none'])), _l = _k.next(); !_l.done; _l = _k.next()) {
                        ...
                        try {
                            for (var _m = (e_4 = void 0, __values([true, false])), _o = _m.next(); !_o.done; _o = _m.next()) {
                                ...
                                WORD_PROCESS_OPTIONS.push({ chanse: chance / 18., proces: new StressPlacement(reverse === "true", Number.parseInt(headSize), attractors, tailMode, lengthen) });
                            }
                        }
            ...
            var minSilabia = Number.parseInt(code);
            for (var bias = -1; bias <= 1; bias++)
                WORD_PROCESS_OPTIONS.push({ chanse: chance / 3, proces: new Syllabicization(bias, minSilabia) });
        }
        else {
            PHRASE_PROCESS_OPTIONS.push({ chanse: chance, proces: new PhraseProcess(type) });
        }
    }
}
