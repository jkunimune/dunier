 ...
 */
import { assert_xy, assert_φλ, endpoint } from "../utilities/coordinates.js";
import { angleSign, arcCenter, lineArcIntersections, lineLineIntersection } from "../utilities/geometry.js";
 ...
 * @returns the relative outputs in transformed coordinates
 */
export function transformOutput(orientation, segments) {
 ...
 */
export function applyProjectionToPath(projection, inPoints, precision) {
            ...
            while (pendingInPoints.length > 0) { // now, as long as there are points in the pending cue
                ...
                if (Math.hypot(nextOutPoint.x - lastOutPoint.x, nextOutPoint.y - lastOutPoint.y) < precision) { // if it's short enuff
                    ...
                    if (pendingInPoints.length + outPoints.length > 100000)
                        throw new Error("why can't I find a point between [".concat(lastInPoint.φ, ",").concat(lastInPoint.λ, "]=>[").concat(lastOutPoint.x, ",").concat(lastOutPoint.y, "] and [").concat(nextInPoint.φ, ",").concat(nextInPoint.λ, "]=>[").concat(nextOutPoint.x, ",").concat(nextOutPoint.y, "]"));
                }
            }
        }
        else {
        ...
        // for segments in the middle of a section
            ...
            if (crossings.length === 0) { // if there's no interruption or we discarded them all
                if (encompasses(edges, [lastSegment], domain) !== encompasses(edges, [lastSegment], domain))
                    throw new Error("you failed to detect a crossing between ".concat(lastSegment.type).concat(lastSegment.args.join(','), " and ").concat(thisSegment.type).concat(thisSegment.args.join(','), "."));
...
export function contains(polygon, point, domain, garanteedToSucced) {
    ...
    for (var i = 1; i < polygon.length; i++) {
        ...
        var intersections = void 0;
        if (polygon[i].type === 'M')
            intersections = [];
        else if (polygon[i].type === 'Λ')
            intersections = [];
        else if (polygon[i].type === 'Φ') {
            var crosses = (start.t < point.t) !== (end.t < point.t);
            if (crosses)
            ...
            var _c = __read(polygon[i].args, 6), radius = _c[0], largeArc = _c[3], sweepDirection = _c[4];
            ...
            intersections = [];
            var discriminant = Math.pow(radius, 2) - Math.pow(point.t - center.y, 2);
            if (discriminant >= 0) {
                try {
                    for (var _d = (e_9 = void 0, __values([-1, 1])), _e = _d.next(); !_e.done; _e = _d.next()) {
                        var sign = _e.value;
                        var x = center.x + sign * Math.sqrt(discriminant);
                        var vy = (sweepDirection > 0) ? x - center.x : center.x - x;
                        if (vy !== 0) { // (ignore tangencies)
                            // make sure the intersection is on or between the endpoints
                            var pointSign = -angleSign(q0, { x: x, y: point.t }, q1); // negate this because of the left-handed coordinate system
            ...
            throw new Error("you may not use shapes with ".concat(polygon[i].type, " segments in contains()"));
        }
        try {
            // look at where it crosses
            for (var intersections_1 = (e_10 = void 0, __values(intersections)), intersections_1_1 = intersections_1.next(); !intersections_1_1.done; intersections_1_1 = intersections_1.next()) {
                var _f = intersections_1_1.value, s = _f.s, goingEast = _f.goingEast;
                var d = s - point.s;
    ...
    for (var i = 0; i < segments.length; i++) { // TODO: this won't notice when the pole is included in the region
        ...
        switch (segment.type) {
            ...
            case 'A':
                ...
                if (chord > 2 * r)
                    throw new Error("this arc is impossible; it needs to span a distance of ".concat(chord, " with a radius of only ").concat(r, "?"));
        ...
        try {
            ...
            for (var points_1 = (e_11 = void 0, __values(points)), points_1_1 = points_1.next(); !points_1_1.done; points_1_1 = points_1.next()) {
    ...
    for (var i = 1; i < edges.length; i++) { // then look at each edge segment
        ...
        // if we're in the infinite cartesian plane
            ...
            try {
                ...
                for (var _b = (e_12 = void 0, __values(getMapEdgeCrossings(assert_xy(segmentStart), segment, assert_xy(edgeStart), edge))), _c = _b.next(); !_c.done; _c = _b.next()) {
                    var point = _c.value;
 ...
 */
function rotatePath(segments, angle) {
    var e_13, _a;
    ...
    try {
        for (var segments_2 = __values(segments), segments_2_1 = segments_2.next(); !segments_2_1.done; segments_2_1 = segments_2.next()) {
            var _b = segments_2_1.value, oldType = _b.type, oldArgs = _b.args;
            var newType = oldType;
