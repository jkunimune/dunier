...
export function delaunayTriangulate(points, normals, sample, sampleNormals, partition) {
    ...
    var partitionTriangles = partition.map(function (t) {
        return new DelaunayTriangle(nodos[t[0]], nodos[t[1]], nodos[t[2]], true);
    });
    ...
    try {
        for (var _g = __values(nodos.slice(sample.length)), _h = _g.next(); !_h.done; _h = _g.next()) { // for each node,
            ...
            for (var j = 0; j < 3; j++) { // add the three new child triangles
                triangles.push(new DelaunayTriangle(node, containing.nodos[j], containing.nodos[(j + 1) % 3]));
            }
    ...
    var triangleIdx = triangles.filter(function (t) { return t.children === null; })
...
function removeNode(node) {
    var allOldTriangles = __spreadArray([], __read(node.triangles), false).filter(function (t) { return t.children === null; });
    ...
    try {
        for (var triangles_2 = __values(triangles), triangles_2_1 = triangles_2.next(); !triangles_2_1.done; triangles_2_1 = triangles_2.next()) {
            ...
            if (contains(triangle, node)) {
                var d2 = /*(triangle.basic) ?*/ distanceSqr(triangle, node); // : 0; // we need to check the distance in case there are multiple candies
                if (d2 < bestDistance)
...
var DelaunayTriangle = /** @class */ (function () {
    function DelaunayTriangle(a, b, c, basic) {
        var e_15, _a;
