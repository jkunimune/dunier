...
var Surface = /** @class */ (function () {
    ...
    Surface.prototype.randomlySubdivide = function (rng) {
        // generate the tiles
        var tiles = []; // remember to clear the old tiles, if necessary
        for (var i = 0; i < Math.max(100, this.area / TILE_AREA); i++) {
            var λ = rng.uniform(-Math.PI, Math.PI);
            var A = rng.uniform(0, this.cumulAreas[this.cumulAreas.length - 1]);
...
var Tile = /** @class */ (function () {
    function Tile(index, position, surface) {
        ...
        var northCylindrical = surface.tangent(this.φ);
        var rHat = this.pos.minus(surface.axis.times(this.pos.dot(surface.axis))).normalized(); // the direction away from the planet's axis
        var zHat = surface.axis;
...
var Vertex = /** @class */ (function () {
    ...
    Vertex.prototype.toString = function () {
        return "".concat((this.tiles[0] instanceof Tile) ? this.tiles[0].pos : 'void', "--") +
            "".concat((this.tiles[1] instanceof Tile) ? this.tiles[1].pos : 'void', "--") +
            "".concat((this.tiles[2] instanceof Tile) ? this.tiles[2].pos : 'void');
    };
    return Vertex;
}());
export { Vertex };
