...
for i, t in enumerate(range(180, 540, 6)): # draw it
	...
	ax.axis([-1, 1, -1, 1])
	plt.savefig('../res/frames/frame{:03d}.png'.format(i), bbox_inches=0, transparent=True)

subprocess.run(['magick', "convert", '-resize', '20%', '-delay', '2', '-dispose', '3', '../res/frames/frame*.png', '../res/lada.mp4']) # and compress it into a GIF
# subprocess.run(['ffmpeg', '-framerate', '1', '-i', '../res/frames/frame%03d.png', '-c:v', 'libx264', '-y', '../res/lada.mp4']) # and compress it into a GIF

shutil.rmtree('../res/frames') # clean up
